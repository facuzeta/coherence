...
class coherenceAnalisys():
	...
	def analysis_text(self,text, max_order=10):
		...
		m = np.array( map(self._unitvec, mean_vectors_series))
		...
		normalized_coeff=[ map(np.mean,zip(len_words_per_vectors[:-i],len_words_per_vectors[i:])) for i in range(1,max_order)]
